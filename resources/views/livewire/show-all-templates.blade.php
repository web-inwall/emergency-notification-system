<div>
    ...
    <div id="all-templates" style="display: none">
        <h3 class="h3-all">Все шаблоны:</h3>
        @if(!empty($templates))
    ...
    <form id="upload-form" action="{{ route('main.manageUserNotificationWorkflow') }}" method="POST"
        enctype="multipart/form-data">
        @csrf
        @if(empty($selectedTemplateName))
        <input type="file" name="file" id="file" required>
        @endif
        <input type="text" name="template_name" placeholder="*Имя шаблона отправки" wire:model="selectedTemplateName"
            required @if(!empty($selectedTemplateName)) readonly @endif>
        <textarea name="message" placeholder="*Введите сообщение" wire:model="selectedTemplateMessage"
            required></textarea>
